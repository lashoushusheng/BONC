...
class Train_Get_DataSource_List_Handler(DataServiceBaseHandler):
    """
        获取-数据源列表
    """
    def post(self):
        ...
        mylog.info("[Train_Get_DataSources.Request]...[%s]/[%s]" % (type(reqData), reqData))
        ...
        mylog.info("[Train_Get_DataSources.Verify]...errno=[%s]...errMsg=[%s]...reqDict=[%s]" % (
        ...
        # [Mysql]，取记录.
        rows = Mysql_MA_DataSource.find_all(
            useType=1, modelType=reqDict.get("modelType")
        )
        ...
        for row in rows:
            # print(row)
            dataSourceList.append(row.dsName)

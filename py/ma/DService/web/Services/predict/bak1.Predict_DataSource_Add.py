...
class Predict_Add_DataSource_Handler(DataServiceBaseHandler):
    ...
    def data_deal(self, dsDir, dsFile):
        cnx = engine.raw_connection()
        data = pd.read_sql('SELECT * FROM dfd_ds_product limit 20', cnx)
        if not data.empty:
            data.to_csv(f"{dsDir}/{dsFile}", sep=",", index=False,
                        columns=dfd_columns)
